...
class HomeWizardEnergy extends Homey.Device {
	...
	startPolling() {
		...
		refreshIntervalIdReadings = setInterval(function () {
			console.log("--Start Homewizard Energy readings Polling-- ");
		...
		homewizard.getDeviceData(homewizard_id, 'energylinks', function(callback){
			...
			if (Object.keys(callback).length > 0) {
				...
				try {
					...
					var energy_current_netto = ( callback[0].active_power_w ); // Netto power usage from aggregated value, this value can go negative

					// Some Energylink do not have gas information so try to get it else fail silently
					try {
						var gas_daytotal_cons = ( callback[0].total_gas_m3 ); // m3 Energy produced via S1 $energylink[0]['gas']['dayTotal']
						// Consumed gas
	...
	getReadings() {
		...
		var me = this;
		homewizard.getDeviceData(homewizard_id, function (callback) {
			...
			if (Object.keys(callback).length > 0) {
				try {
					...
					// Error with Energylink no data in Energylink
					console.log("No Homewizard Energy data found");
					me.setUnavailable();
				}
			}
